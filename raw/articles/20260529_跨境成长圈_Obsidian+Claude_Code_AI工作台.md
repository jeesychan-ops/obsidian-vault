---
title: "用 Obsidian + Claude Code 搭一个 AI 工作台：自动整理待办、热点和选题"
source: "跨境成长圈"
date: "2026-05-29"
...
# 用 Obsidian + Claude Code 搭一个 AI 工作台：自动整理待办、热点和选题
...
之前有分享过如何配置 Obsidian，接下来分享如何利用这个工具。
很多自媒体创作者每天都会被这些小事打断：
• 昨天还有哪些事情没做完？
• 今天有哪些热点值得关注？
• 哪个想法可以变成一篇文章？
...
1. 点击“打开文件夹作为仓库”
2. 选择刚才下载的 OrbitOS-vault 文件夹
3. 等 Obsidian 加载完成
到这里，你已经拥有了一个带工作流程的 Obsidian 资料库。接下来要做的是：让 AI 能在这个资料库里工作。
这里有两种方式。你可以根据自己的情况选择。
方式 更适合谁 优点 注意点 Claudian 插件 想少折腾的新手 设置更像普通软件插件，操作友好 通常需要使用 Claude API，费用可能更高 Terminal 插件 + Claude Code 愿意折腾一点的人 更灵活，也可以接入其他模型服务 需要先安装并配置 Claude Code
Claudian 插件
设置更像普通软件插件，操作友好
通常需要使用 Claude API，费用可能更高
Terminal 插件 + Claude Code
更灵活，也可以接入其他模型服务
需要先安装并配置 Claude Code
...
如果插件市场里找不到，可以手动安装：
1. 打开发布页面：github.com/YishenTu/claudian/releases [2]
2. 下载 main.js 、 manifest.json 、 styles.css
3. 在 Obsidian 中打开插件文件夹
4. 新建一个名为 claudian 的文件夹
5. 把这三个文件放进去
6. 重启 Obsidian，并在社区插件里启用它
...
4. 需要查资料时，运行 Research ，让 AI 帮你整理背景信息。
